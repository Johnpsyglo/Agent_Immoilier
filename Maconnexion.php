<?php
 ...
 class MaConnexion{
    ...
    public function select_potentiel_acheteur($table, $column){
        try {
            // On reparamètre notre requete SQL selon nos besoin
            // Ici on avait besoin de selectionné 1 réserver_maison donc on rajoute ORDERBY BY 'nom_column' DESC pour mettre notre liste en ordre décroissant
            // Et ensuite LIMIT 1 pour limiter le nombre réserver_maison selectionné à 1
            $requete = "SELECT $column from $table ORDER BY id_acheteur DESC LIMIT 1 ";
            $resultat = $this->connexionPDO->query($requete);
            $resultat = $resultat->fetchAll(PDO::FETCH_ASSOC); //Recupere le resultat de la requete dans un tableau associatif
            return $resultat;
     ...
     // On modifie juste le nom de la function ici "select" devient "select_articles_recents"
     public function select_réserver_maison($table, $column){
        try {
            // On reparamètre notre requete SQL selon nos besoin
            ...
            $requete = "SELECT $column from $table WHERE categorie = 1 ORDER BY id_contact DESC LIMIT 2 ";
            $resultat = $this->connexionPDO->query($requete);
            $resultat = $resultat->fetchAll(PDO::FETCH_ASSOC); //Recupere le resultat de la requete dans un tableau associatif
            return $resultat;
     ...
     public function select_maison_spacieux($table, $column){
        try {
            // On reparamètre notre requete SQL selon nos besoin
            // Ici on avait besoin de selectionné 3 articles récentes donc on rajoute ORDERBY BY 'nom_column' DESC pour mettre notre liste en ordre décroissant
            // Et ensuite LIMIT 3 pour limiter le nombre d'article selectionné à 3
